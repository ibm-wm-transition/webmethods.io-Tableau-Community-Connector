  ...
  execute: function(input, options, output) {
    ...
    self
      ...
      .then(res => {
        ...
        if (
          res &&
          Array.isArray(res.tsResponse.groups.group) &&
          res.tsResponse.groups.group.length
        ) {
          res.tsResponse.groups.group.forEach(group => {
            if (group) {
              arr.push({
                id: String(group._attributes.id),
                value:
                  String(group._attributes.name) || String(group._attributes.id)
              });
            }
          });
